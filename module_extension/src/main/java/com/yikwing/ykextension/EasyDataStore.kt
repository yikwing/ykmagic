...
class EasyDataStore private constructor(private var dataStore: DataStore<Preferences>) {

    companion object {
        // 单例集合
        private val sSPMap = HashMap<String, EasyDataStore>()

        @Volatile
        private var instance: EasyDataStore? = null

        fun getInstance(pbName: String, dataStore: DataStore<Preferences>): EasyDataStore {
            instance = sSPMap[pbName]
            return instance ?: synchronized(this) {
                instance ?: EasyDataStore(dataStore).also {
                    sSPMap[pbName] = it
                    instance = it
                }
            }
